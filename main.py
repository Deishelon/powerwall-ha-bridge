...
def fetch_pw_data(pw_api: pypowerwall.Powerwall, mqtt: MqttClient, logger: Logger):
    # INFO:powerwall2mqtt:bat=-2292
    # Negative=charging, Positive=discharging
    logger.info(f"bat={pw_api.battery()}")

    # INFO:powerwall2mqtt:bat={'last_communication_time': '2026-03-20T13:57:17-07:00', 'instant_power': -2292, 'instant_reactive_power': 0, 'instant_apparent_power': 0, 'frequency': 0, 'energy_exported': 0, 'energy_imported': 0, 'instant_average_voltage': 238.4, 'instant_average_current': -9.614093959731543, 'i_a_current': 0, 'i_b_current': 0, 'i_c_current': 0, 'last_phase_voltage_communication_time': '0001-01-01T00:00:00Z', 'last_phase_power_communication_time': '0001-01-01T00:00:00Z', 'last_phase_energy_communication_time': '0001-01-01T00:00:00Z', 'timeout': 1500000000, 'num_meters_aggregated': 1, 'instant_total_current': -9.614093959731543, 'disclaimer': 'battery: voltage from PINV, calculated current from power'}
    logger.info(f"bat={pw_api.battery(verbose=True)}")

    # INFO:powerwall2mqtt:level=60.41666666666667
    logger.info(f"level={pw_api.level()}")

    # INFO:powerwall2mqtt:level=58.33333333333334
    logger.info(f"level={pw_api.level(scale=True)}")

    # INFO:powerwall2mqtt:load=401.75000000000006
    logger.info(f"load={pw_api.load()}")

    # INFO:powerwall2mqtt:load={'last_communication_time': '2026-03-20T13:58:58-07:00', 'instant_power': 401.75000000000006, 'instant_reactive_power': 0, 'instant_apparent_power': 0, 'frequency': 0, 'energy_exported': 0, 'energy_imported': 0, 'instant_average_voltage': 238.5, 'instant_average_current': 1.6844863731656188, 'i_a_current': 0, 'i_b_current': 0, 'i_c_current': 0, 'last_phase_voltage_communication_time': '0001-01-01T00:00:00Z', 'last_phase_power_communication_time': '0001-01-01T00:00:00Z', 'last_phase_energy_communication_time': '0001-01-01T00:00:00Z', 'timeout': 1500000000, 'instant_total_current': 1.6844863731656188, 'num_meters_aggregated': 1, 'disclaimer': 'load: voltage from ISLAND_AcMeasurements, current calculated from power'}
    logger.info(f"load={pw_api.load(verbose=True)}")
    ...
    # INFO:powerwall2mqtt:site=5
    logger.info(f"site={pw_api.site()}")

    # INFO:powerwall2mqtt:site={'last_communication_time': '2026-03-20T14:05:19-07:00', 'instant_power': 5, 'instant_reactive_power': 0, 'instant_apparent_power': 0, 'frequency': 0, 'energy_exported': 0, 'energy_imported': 0, 'instant_average_voltage': 239, 'instant_average_current': 0.02092050209205021, 'i_a_current': 8.777000000000001, 'i_b_current': 0, 'i_c_current': 0, 'last_phase_voltage_communication_time': '0001-01-01T00:00:00Z', 'last_phase_power_communication_time': '0001-01-01T00:00:00Z', 'last_phase_energy_communication_time': '0001-01-01T00:00:00Z', 'timeout': 1500000000, 'num_meters_aggregated': 1, 'instant_total_current': 0.02092050209205021, 'disclaimer': 'site: voltage/current from Meter X'}
    logger.info(f"site={pw_api.site(verbose=True)}")

    # INFO:powerwall2mqtt:solar=2992
    logger.info(f"solar={pw_api.solar()}")

    # INFO:powerwall2mqtt:solar={'last_communication_time': '2026-03-20T14:05:19-07:00', 'instant_power': 2992, 'instant_reactive_power': 0, 'instant_apparent_power': 0, 'frequency': 0, 'energy_exported': 0, 'energy_imported': 0, 'instant_average_voltage': 238.20000000000002, 'instant_average_current': 12.560873215785053, 'i_a_current': 0, 'i_b_current': 0, 'i_c_current': 0, 'last_phase_voltage_communication_time': '0001-01-01T00:00:00Z', 'last_phase_power_communication_time': '0001-01-01T00:00:00Z', 'last_phase_energy_communication_time': '0001-01-01T00:00:00Z', 'timeout': 1000000000, 'num_meters_aggregated': 1, 'instant_total_current': 12.560873215785053, 'disclaimer': 'solar: voltage from PVAC, calculated current from power'}
    logger.info(f"solar={pw_api.solar(verbose=True)}")

    # INFO:powerwall2mqtt:temps={}
    logger.info(f"temps={pw_api.temps()}")
    ...
    # INFO:powerwall2mqtt:version=26.2.1 a7456b0a
    logger.info(f"version={pw_api.version()}")
...
def main():
    ...
    try:
        while True:
            logger.info("Polling...")
            fetch_pw_data(pw_api, mqtt, logger)
